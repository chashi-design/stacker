...
- **原因**: `exerciseName`削除＋`exerciseId`非Optional化のようなスキーマ変更で既存ストアが読み込めず、起動時にストアが再生成されると「アプリキルでデータが消える」症状が出る。
- **対策**: 変更前に「移行で旧データを保持するか／破棄するか」を明示する。
- 旧データ破棄の場合は、起動時にストア読み込み失敗を検知して削除→再生成する処理を入れる（再起動で保持されることを確認）。
- 旧データ保持の場合は、SwiftDataの互換性を維持できる変更に留めるか、明示的な移行フローを用意する。

## Data Persistence Safety Rules (No Data Loss)
- **破壊的変更禁止**: 既存モデルのフィールド削除/型変更/Optional→非Optional変更は、移行実装なしでは実施しない。
- **必須: 移行方針の明記**: 変更前に「データ保持 or 破棄」を明示し、保持の場合は移行コードを用意する。
- **必須: 事前バックアップ**: 保存データをJSON等でエクスポートできる仕組みを検討し、破壊的変更前にバックアップを取得する。
- **必須: リリース前検証**: 旧ストアを持つ端末でアップデート→起動→再起動でデータが維持されることを確認する。
- **フォールバック禁止**: 読み込み失敗時のストア削除は「旧データ破棄を宣言した場合のみ」許可する。
